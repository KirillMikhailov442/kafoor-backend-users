...
public class UserService implements UserDetailsService {
    ...
    public UserEntity updateUser(UserUpdate dto, long id){
        ...
        if (userRepo.existsByEmail(dto.getEmail()) && userEntity.getId() != id) throw new Conflict("A user with such email already exists");
        if (userRepo.existsByNickname(dto.getNickname()) && userEntity.getId() != id) throw new Conflict("A user with such nickname already exists");

        if (dto.getName() != null && !dto.getName().isEmpty()) userEntity.setName(dto.getName());
        if (dto.getEmail() != null && !dto.getEmail().isEmpty()) userEntity.setEmail(dto.getEmail());
        if (dto.getNickname() != null && !dto.getNickname().isEmpty()) userEntity.setNickname(dto.getNickname());
    ...
    public UserLogined login(UserLogin dto, HttpHeader header){
        ...
        if (!encoder.matches(dto.getPassword(), user.getPasswordHash())) throw new BadRequest("Incorrect password");
